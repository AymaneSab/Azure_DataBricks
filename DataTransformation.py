...
for f_raw in raw_files:
    ...
    if f_raw.name not in processed_files_csv:
        # Read the raw data and concatenate it
        raw_data = dbutils.fs.head(f_raw.path, 1000000)  # Read the first 1 MB of the file (adjust as needed)
        concatenated_raw_data += raw_data
...
def dataTransformationFunction(file_location):
    ...
    file_location = "wasbs://public-transport-data@sabrilakestorage.blob.core.windows.net/raw/publicTransportData.csv"
    file_type = "csv"   
...
# MAGIC %md
# MAGIC
# MAGIC ### Step 3: Apply the transformation 
# MAGIC
# MAGIC
...
# MAGIC %md 
# MAGIC
# MAGIC #### Save the dataframe on the processed container on the data lake 

# COMMAND ----------

# MAGIC %md 
# MAGIC
# MAGIC ##### Mount To Azure Data Lake 
...
pandas_df.to_csv(f"/dbfs/mnt/data/public-transport-data/raw/Processed.csv", index = False)
